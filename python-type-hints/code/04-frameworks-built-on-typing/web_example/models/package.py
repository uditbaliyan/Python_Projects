...
class Package(beanie.Document):
    ...
    class Settings:
        ...
        indexes = [
            ...
            pymongo.IndexModel(
                keys=[("releases.created_date", pymongo.ASCENDING)],
                name="releases_created_date_ascend",
            ),
            pymongo.IndexModel(
                keys=[("author_email", pymongo.ASCENDING)], name="author_email_ascend"
            ),
            pymongo.IndexModel(
                keys=[("releases.major_ver", pymongo.ASCENDING)], name="releases_major"
            ),
            pymongo.IndexModel(
                keys=[("releases.minor_ver", pymongo.ASCENDING)], name="releases_minor"
            ),
            pymongo.IndexModel(
                keys=[("releases.build_ver", pymongo.ASCENDING)], name="releases_build"
            ),
            pymongo.IndexModel(
                keys=[
